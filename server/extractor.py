...
class InstructorExtractor:
    ...
    async def extract_base_content(self, markdown: str) -> BaseContentModel:
        response = await self.client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "Extract basic content from the markdown of a social media post."},
    ...
    async def extract_links(self, html: str) -> LinksModel:
        response = await self.client.chat.completions.create(
            model=self.fast_model,
            messages=[
                {"role": "system", "content": "Extract links from the html of a social media post main body."},
    ...
    async def extract_metadata(self, html: str) -> MetadataModel:
        response = await self.client.chat.completions.create(
            model=self.fast_model,
            messages=[
                {"role": "system", "content": "Extract metadata from the html of a social media post."},
                {"role": "user", "content": html}
            ],
    ...
    async def extract_comments(self, html: str) -> list[CommentsModel]:
        try:
            response = await self.client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "Extract at most 10 comments from the html of a social media post. For each comment, extract the comment content, author name, publish date, and first reply if any."},
                    {"role": "user", "content": html}
                ],
    ...
    async def extract_author_profile(self, markdown: str) -> AuthorProfileModel:
        response = await self.client.chat.completions.create(
            model=self.fast_model,
            messages=[
                {"role": "system", "content": "Extract author profile from the markdown of a social media profile page."},
                {"role": "user", "content": markdown}
            ],
    ...
    async def extract_note(self, url: Optional[str] = None, html: Optional[str] = None) -> Dict[str, Any]:
    ...
    async def extract_profile(self, url: Optional[str] = None, html: Optional[str] = None) -> Dict[str, Any]:
        if not url and not html:
            raise ValueError("Either url or html must be provided")
