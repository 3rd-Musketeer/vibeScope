...
class QueryResponse(BaseModel):
    ...
    relevant_note_ids: list[str] = Field(
        description="IDs of notes that were relevant to the question"
    )
...
class ExtractedContentResponse(BaseModel):
    ...
    processing_time_seconds: int = Field(
        description="Processing time excluding wait time"
    )
...
if __name__ == "__main__":
    ...
    task_request = TaskCreateRequest(
        project_id="test-project", url="https://example.com/post/123"
    )
    ...
    project = ProjectSchema(
        id="test-project", name="Test Project", created_at=datetime.now()
    )
    ...
    print(
        f"✓ ExtractedContentResponse transformation passed: {api_response.base_content.title}"
    )
    print(
        f"✓ Nested structure: author='{api_response.author_profile.author_name}', likes={api_response.metadata.like_count}"
    )
